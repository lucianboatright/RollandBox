...
export default function User({ username, fullName, avatar }) {
  return !username || !fullName ? (
    ...
    <Link to={`/${username}`} className="grid grid-cols-4 gap-4 mb-3 pb-3 items-center border-b-2">
      <div className="items-center pr-1 col-span-1 w-10 invisible sm:invisible h-10 w-10 md:visible w-10 h-10 lg:visible h-10 w-10 xl:visible w-10 h-10">
        <img
          className="rounded-full mr-3 shadow-lg invisible sm:invisible md:invisible lg:visible xl:visible"
          src={avatar}
          alt={username}
        />
      </div>
